# ADR-001: Keep handler-traffic as an Independent Gnalloy Module
...
`handler-traffic` is published as `gnalloy.org/handler-traffic` and owns this responsibility: Traffic shaping and rate limiting handlers for Gnalloy pipelines.
